...
const documents: DocumentRow[] = [
  { name: "DMA_진도점검_발표자료.pdf", type: "기술자료", status: "활성", rag: "완료", date: "2026.08.31" },
  { name: "DMA_Retrofit_기획자료.pdf", type: "기술자료", status: "활성", rag: "완료", date: "2026.08.29" },
  { name: "Winch_운용자료_샘플.pdf", type: "매뉴얼", status: "활성", rag: "처리 중", date: "2026.08.28" },
  { name: "Sensor_Reference.pdf", type: "참고자료", status: "비활성", rag: "실패", date: "2026.08.26" },
];
...
export default function AdminPage() {
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((item) => `${item.name} ${item.type}`.toLowerCase().includes(q));
  }, [search]);

  const titleMap: Record<Section, [string, string]> = {
    dashboard: ["대시보드", "DMA Assistant의 지식과 서비스 상태를 빠르게 확인합니다."],
    knowledge: ["지식 관리", "문서, 지정답변, 키워드를 한 곳에서 관리합니다."],
    ...
    <main className={styles.shell}>
      ...
      <section className={styles.workspace}>
        <header className={styles.topbar}>
          <div>
            <span className={styles.productName}>DMA Assistant Admin</span>
            <span className={styles.environment}>1차년도 MVP · UI Preview</span>
          </div>
          <div className={styles.topbarRight}>
            <span className={styles.connection}><span /> Mock Data</span>
        ...
        <div className={styles.page}>
          <div className={styles.pageHeading}>
            <div>
              <h1>{titleMap[section][0]}</h1>
              <p>{titleMap[section][1]}</p>
            </div>
            {section === "knowledge" && (
            ...
            {section === "images" && (
              <button className={styles.primaryButton} onClick={() => openDrawer("image")}><Upload size={17} /> 이미지 등록</button>
            )}
          </div>
          ...
          {section === "operations" && (
            <Operations
              tab={operationTab}
              setTab={setOperationTab}
              onOpenFeedback={() => openDrawer("feedback")}
              testQuestion={testQuestion}
              setTestQuestion={setTestQuestion}
              testRan={testRan}
              onRun={() => setTestRan(true)}
            />
          )}
          {section === "settings" && (
            <SystemSettings
  ...
  return (
    <div className={styles.dashboardGrid}>
      <div className={styles.metricGrid}>
        <Metric icon={FileText} label="등록 문서" value="4" note="RAG 완료 2건" />
        ...
        <Metric icon={CircleAlert} label="처리 실패" value="1" note="문서 재처리 필요" tone="red" />
      </div>

      <section className={styles.card}>
        <div className={styles.cardHeader}><div><h2>최근 활동</h2><p>Mock 데이터 기준 최근 관리자 작업</p></div></div>
        <div className={styles.activityList}>
          <Activity icon={Upload} title="DMA 진도점검 발표자료 등록" meta="지식 관리 · 방금 전" />
          <Activity icon={RefreshCw} title="Winch 운용자료 RAG 처리 시작" meta="지식 관리 · 12분 전" />
      ...
      <section className={styles.card}>
        ...
        <div className={styles.serviceList}>
          <ServiceLine icon={Sparkles} label="OpenAI API" value="미연결" tone="gray" />
          <ServiceLine icon={Database} label="Vector DB" value="미연결" tone="gray" />
          <ServiceLine icon={ImageIcon} label="Storage" value="미연결" tone="gray" />
        </div>
      </section>
      ...
      <section className={`${styles.card} ${styles.quickCard}`}>
        ...
        <button onClick={() => onMove("knowledge")}><FileText size={19} /><span><strong>지식 등록</strong><small>문서·지정답변·키워드</small></span><ChevronRight size={18} /></button>
...
function Metric({ icon: Icon, label, value, note, tone = "blue" }: { icon: typeof FileText; label: string; value: string; note: string; tone?: "blue" | "orange" | "red" }) {
  return (
    <div className={styles.metricCard}>
      <div className={`${styles.metricIcon} ${styles[`metric_${tone}`]}`}><Icon size={20} /></div>
...
function ServiceLine({ icon: Icon, label, value, tone }: { icon: typeof Sparkles; label: string; value: string; tone: "gray" | "green" }) {
  return <div className={styles.serviceLine}><div><Icon size={18} /><span>{label}</span></div><Status tone={tone}>{value}</Status></div>;
}
    ...
    <section className={styles.card}>
      ...
      {tab === "documents" && (
        <div className={styles.tableWrap}>
          <table><thead><tr><th>문서명</th><th>유형</th><th>상태</th><th>RAG</th><th>등록일</th><th /></tr></thead>
            <tbody>{filteredDocuments.map((item) => (
              <tr key={item.name}><td><FileText size={15} />{item.name}</td><td>{item.type}</td><td><Status tone={item.status === "활성" ? "green" : "gray"}>{item.status}</Status></td><td><Status tone={item.rag === "완료" ? "green" : item.rag === "실패" ? "red" : "blue"}>{item.rag}</Status></td><td>{item.date}</td><td><button className={styles.textButton} onClick={() => onOpen("document")}>보기</button></td></tr>
            ))}</tbody></table>
        </div>
      )}
  ...
  return (
    <>
      <div className={styles.toolbarStandalone}>
        <label className={styles.searchBox}><Search size={17} /><input placeholder="이미지 제목, 키워드 검색" /></label>
        <button className={styles.secondaryButton}><SlidersHorizontal size={16} /> 전체 분류</button>
      </div>
...
function Operations({ tab, setTab, onOpenFeedback, testQuestion, setTestQuestion, testRan, onRun }: {
  ...
  return (
    <section className={styles.card}>
      <div className={styles.tabs}>
        <button className={tab === "feedback" ? styles.tabActive : ""} onClick={() => setTab("feedback")}>사용자 피드백</button>
        <button className={tab === "test" ? styles.tabActive : ""} onClick={() => setTab("test")}>응답 테스트</button>
      </div>
      ...
      {tab === "feedback" ? (
        ...
        <div className={styles.testLayout}>
          <div className={styles.testInput}>
            <label>테스트 질문</label>
            <textarea value={testQuestion} onChange={(e) => setTestQuestion(e.target.value)} />
            <button className={styles.primaryButton} onClick={onRun}><Play size={17} /> 답변 테스트</button>
          </div>
          <div className={styles.testResult}>
            {!testRan ? (
              <div className={styles.emptyState}><Bot size={30} /><strong>테스트 결과가 여기에 표시됩니다.</strong><span>현재는 Mock 결과만 확인할 수 있습니다.</span></div>
            ) : (
              <>
                <div className={styles.resultSummary}><Status tone="blue">설명·절차형</Status><span>키워드: DMA, Retrofit</span><Status tone="green">RAG Mock</Status></div>
                <h3>DMA Retrofit 적용 검토</h3>
                <p>현재 등록된 Mock 지식 기준으로, 기존 선박의 계류·묘박 장비와 제어·전력·통신 연계 구조를 사전 검토한 뒤 Retrofit 범위를 결정하는 흐름으로 답변합니다.</p>
                <div className={styles.sourceMini}><FileText size={16} /><span><strong>참조 자료</strong> DMA 진도점검 발표자료 · Mock</span></div>
              </>
            )}
          </div>
        </div>
...
function SystemSettings({ apiKey, setApiKey, model, setModel, ragEnabled, setRagEnabled, imageEnabled, setImageEnabled }: {
  ...
  return (
    <div className={styles.settingsGrid}>
      ...
      <section className={styles.card}>
        <div className={styles.cardHeader}><div><h2>AI 응답 설정</h2><p>1차년도에 필요한 최소 항목만 노출합니다.</p></div><SlidersHorizontal size={20} /></div>
        ...
        <Toggle label="RAG 사용" description="등록 문서를 검색해 답변 근거로 활용" checked={ragEnabled} onChange={setRagEnabled} />
        <Toggle label="관련 이미지 제공" description="키워드가 연결된 이미지를 답변에 표시" checked={imageEnabled} onChange={setImageEnabled} />
        <div className={styles.formGroup}><label>검색 문서 수</label><input defaultValue="5" /></div>
        <div className={styles.formGroup}><label>근거 부족 시 정책</label><select defaultValue="limit"><option value="limit">근거 부족 안내 후 답변 제한</option><option value="general">일반 안내 답변 제공</option></select></div>
      </section>

      <section className={`${styles.card} ${styles.noticeCard}`}>
        <ShieldCheck size={21} />
        <div><strong>현재는 UI 단계입니다.</strong><p>API Key, 모델 설정, 토글 상태는 서버나 DB에 저장되지 않습니다. 실제 연결 단계에서 Supabase/Vault 및 서버 API를 추가합니다.</p></div>
      </section>
    </div>
  );
}
...
function AdminDrawer({ type, onClose }: { type: "document" | "answer" | "keyword" | "image" | "feedback"; onClose: () => void }) {
  const title = { document: "문서 등록 / 상세", answer: "지정답변 등록 / 수정", keyword: "키워드 등록 / 수정", image: "이미지 등록 / 수정", feedback: "피드백 상세" }[type];
  return (
    <div className={styles.drawerBackdrop} onMouseDown={onClose}>
      <aside className={styles.drawer} onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className={styles.drawerFooter}><button className={styles.secondaryButton} onClick={onClose}>취소</button><button className={styles.primaryButton} onClick={onClose}><Check size={16} /> Mock 저장</button></div>
      </aside>
    </div>
  );
}
...
function DocumentForm() {
  return <div className={styles.drawerBody}><div className={styles.uploadZone}><Upload size={24} /><strong>문서 파일 선택</strong><span>PDF / DOCX / TXT · 현재 업로드 동작 없음</span></div><Field label="문서명" placeholder="DMA Operating Manual" /><Field label="문서 유형" placeholder="기술자료" /><Field label="버전" placeholder="v1.0" /><Field label="키워드" placeholder="DMA, Winch, Retrofit" /><div className={styles.mockProcess}><span><Check size={14} /> 업로드</span><span>→</span><span>텍스트 추출</span><span>→</span><span>RAG 처리</span></div></div>;
}
...
function KeywordForm() {
  return <div className={styles.drawerBody}><Field label="대표 키워드" placeholder="Winch" /><Field label="동의어 / 유사어" placeholder="윈치, Mooring Winch" /><Field label="연결 질문 유형" placeholder="설명형, 문제해결형" /><Field label="우선 지정답변" placeholder="선택 안 함" /></div>;
}

function ImageForm() {
  return <div className={styles.drawerBody}><div className={styles.uploadZone}><ImageIcon size={24} /><strong>이미지 선택</strong><span>JPG / PNG · 현재 업로드 동작 없음</span></div><Field label="제목" placeholder="DMA 시스템 구성도" /><Field label="설명" placeholder="답변에 함께 표시할 이미지 설명" /><Field label="키워드" placeholder="DMA, 시스템 구성" /><Field label="관련 문서" placeholder="DMA 진도점검 발표자료" /></div>;
}

function FeedbackDetail() {
  return <div className={styles.drawerBody}><div className={styles.feedbackQuestion}><span>사용자 질문</span><strong>DMA Degree 1.5의 세부 기준은?</strong></div><div className={styles.feedbackQuestion}><span>피드백 사유</span><div><Status tone="orange">정보가 부족함</Status> <Status tone="red">근거가 부족함</Status></div></div><div className={styles.formGroup}><label>사용자 상세 의견</label><p className={styles.readonlyBox}>세부 기능 기준이 정확히 어디까지인지 더 구체적인 자료가 필요합니다.</p></div><div className={styles.formGroup}><label>처리 상태</label><select defaultValue="review"><option value="new">미확인</option><option value="review">검토중</option><option value="done">개선완료</option></select></div></div>;
}
